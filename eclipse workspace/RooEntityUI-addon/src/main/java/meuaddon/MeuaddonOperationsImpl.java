...
@Service
public class MeuaddonOperationsImpl implements MeuaddonOperations {
	...
	public void annotateType(JavaType javaType) {
		...
		if (existing != null && MemberFindingUtils.getAnnotationOfType(existing.getAnnotations(), new JavaType(RooEntityUi.class.getName())) == null) {
			...
			JavaType rooRooMeuaddon = new JavaType(RooEntityUi.class.getName());

			// Create Annotation metadata
			AnnotationMetadataBuilder annotationBuilder = new AnnotationMetadataBuilder(rooRooMeuaddon);
	...
	public void setup() {
		...
		projectOperations.addRepository("", new Repository("Meuaddon Roo add-on repository", "Meuaddon Roo add-on repository", "https://meuaddon.googlecode.com/svn/repo"));
		
		List<Dependency> dependencies = new ArrayList<Dependency>();
		
		// Install the dependency on the add-on jar (
		dependencies.add(new Dependency("meuaddon", "meuaddon", "0.1.0.BUILD-SNAPSHOT", DependencyType.JAR, DependencyScope.PROVIDED));
